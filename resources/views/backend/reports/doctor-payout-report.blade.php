        ...
        <x-backend.section-header>
            <x-slot name="toolbar">
                <div class="input-group gap-2 flex-nowrap">
                    <input type="text" name="appointment_date" id="appointment_date" placeholder="{{ __('setting_sidebar.select_date') }}" class="appointment-date-range form-control form-control-sm ps-3" readonly />
                    <select id="column_employee_id" placeholder="Select Doctor" name="column_employee_id"
...
@push('after-scripts')
    ...
    <script type="text/javascript" defer>
        const range_flatpicker = document.querySelectorAll('.appointment-date-range')
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            initDatatable({
                url: '{{ route('backend.reports.doctor-payout-report.index_data') }}',
                finalColumns,
                advanceFilter: () => {
                    return {
