<template>
  ...
  import { OBSERVATION_EDIT_URL, OBSERVATION_UPDATE_URL, OBSERVATION_STORE_URL } from '../constant/constant'
  ...
  const currentId = useModuleId(() => {
    if (currentId.value > 0) {
      getRequest({ url: OBSERVATION_EDIT_URL, id: currentId.value }).then((res) => {
        if (res.status) {
  ...
  const formSubmit = handleSubmit((values) => {
    ...
    if (currentId.value > 0) {
      updateRequest({ url: OBSERVATION_UPDATE_URL, id: currentId.value, body: values, type: 'file' }).then((res) => reset_datatable_close_offcanvas(res))
    } else {
      storeRequest({ url: OBSERVATION_STORE_URL, body: values, type: 'file' }).then((res) => reset_datatable_close_offcanvas(res))
    }
