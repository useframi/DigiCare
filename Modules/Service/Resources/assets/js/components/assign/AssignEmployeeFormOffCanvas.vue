    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="service-employee-assign-form" aria-labelledby="form-offcanvasLabel">
        ...
        <div class="form-group">
          <div class="d-grid">
            <div class="d-flex flex-column">
              <div class="mb-4">
                <!-- <Multiselect id="service_provider_id" placeholder="Select Service Provider" v-model="service_provider_id" :value="service_provider_id" v-bind="singleSelectOption" :options="service_providers.options" @select="serviceProviderSelect" class="form-group mb-3"></Multiselect> -->
      ...
      <div class="offcanvas-footer">
        <p class="text-center mb-0">
          <small>{{ $t('service.assign_staff_to_service') }}</small>
        </p>
...
import { POST_EMPLOYEE_ASSIGN_URL, GET_EMPLOYEE_ASSIGN_URL, EDIT_URL, EMPLOYEE_LIST, SERVICE_PROVIDER_LIST } from '../../constant/service'
...
const service_providers = ref({ options: [], list: [] })
...
const service_provider_id = ref(null)
...
onMounted(() => {
  useSelect({ url: SERVICE_PROVIDER_LIST }, { value: 'id', label: 'name' }).then((data) => {
    service_providers.value = data
    if (data.options.length === 1) {
      service_provider_id.value = data.options[0].value
    }
  })
  serviceProviderSelect(service_provider_id.value ?? '')
})

// On Select
const serviceProviderSelect = (value) => {
  useSelect({ url: EMPLOYEE_LIST, data: { service_provider_id: value } }, { value: 'id', label: 'name' }).then((data) => (employee.value = data))
}
