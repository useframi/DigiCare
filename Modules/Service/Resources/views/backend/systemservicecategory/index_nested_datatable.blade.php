...
@section('content')
    <div class="table-content mb-3">
      <x-backend.section-header>
          <div>
            <x-backend.quick-action url='{{route("backend.systemcategory.bulk_action")}}'>
            <div class="">
            ...
            <x-slot name="toolbar">
              ...
              @hasPermission('add_system_category')
                <button type="button" class="btn btn-primary" data-crud-id="{{0}}" title="{{ __('messages.create') }} {{ __($module_title) }}"><i class="fas fa-plus-circle me-2"></i>{{ __('messages.create') }} {{ __('category.sub_categories') }}</button>
              @endhasPermission
...
@push('after-scripts')
    ...
    const columns = [
        ...
        document.addEventListener('DOMContentLoaded', (event) => {
            initDatatable({
                url: '{{ route("backend.systemcategory.index_nested_data") }}',
                finalColumns,
                advanceFilter: () => {
                  return {
