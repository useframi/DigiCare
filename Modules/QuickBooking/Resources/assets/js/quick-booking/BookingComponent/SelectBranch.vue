<template>
    <div class="card-list-data">
        <div class="row row-cols-1 row-cols-md-2" v-if="!IS_LOADER">
            <div class="col" v-for="(item, index) in clinicList" :key="item">
                <div class="iq-widget">
                    <input type="radio" :id="item.name + item.id" name="radio" :value="item.id" v-model="clinic_id" class="btn-check" @change="onChange"/>
                    <label :for="item.name + item.id" class="d-block w-100">
                        <div class="card iq-branch-box">
                            <div class="card-body">
                                <div class="text-center mb-4">
                                    <div class="branch-image">
                                        <img :src="item.clinic_image" class="avatar-70 rounded-circle" alt="feature-image" loading="lazy">
                                    </div>
                                    <div class="branch-content mt-3 mt-sm-0">
                                        <h4 class="mb-1">{{ item.name || 'No clinic' }}</h4>
                                        <p class="m-0">{{ item.address || '-' }} / {{ item.pincode || '--' }}</p>
                                    </div>
                                </div>
                                <hr>
                                <ul class="iq-contact-detail list-inline p-0 m-0">
                                    ...
                                    <li class="d-flex align-items-center justify-content-center">
                                        ...
                                        <p class="ms-2 mb-0">{{ item.email || '-' }}</p>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <div class="h-100 w-75 d-flex align-items-center justify-content-center" v-if="clinicList.length == 0 && !IS_LOADER">
          We apologize, but currently, there are no available Clinics for booking appointments.
        </div>
    </div>
    <div class="card-footer">
      ...
      <button type="button" class="btn btn-primary iq-text-uppercase" :disabled="clinic_id !== null ? false : true" v-if="wizardNext" @click="nextTabChange(wizardNext)">Next</button>
    </div>
</template>
<script setup>
// Library Import
...
// URL Constant Import
import { CLINIC_LIST } from '@/vue/constants/quick_booking'
...
const clinic_id = ref(null)
const IS_LOADER = ref(true)


const getClinic = () => {
 IS_LOADER.value = true
 if(store.booking.services[0].service_id !== null || props.user_id !== null) {
   listingRequest({ url: CLINIC_LIST, data: { user_id: props.user_id, service_id: store.booking.services[0].service_id}}).then((res) => {
...
// Reset Data Function
const resetData = () => {
    clinic_id.value = null
}

// Watch
watch(() => clinic_id.value, (value) => {
    
  store.updateBookingValues(
    {key: 'clinic_id',value: value}
  )
}, {deep: true})
