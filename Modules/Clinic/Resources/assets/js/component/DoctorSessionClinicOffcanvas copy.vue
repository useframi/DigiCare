          ...
          <div class="col-md-6">
            <div class="form-group">
              ...
              <Multiselect id="clinic_id" v-model="clinic_id" :value="clinic_id" placeholder="Select Clinic"
                v-bind="singleSelectOption" :options="cliniclist.options" @select="ClinicSelect" class="form-group">
              </Multiselect>
            </div>
          </div>
          <div class="col-md-12">
...
import { CLINIC_LIST, LISTING_URL, STORE_URL, DOCTOR_DATA, DOCTOR_LIST, EDIT_URL, CLINIC_SESSION_LIST } from '../constant/doctor-session'
...
const getClinicList = (doctorId) => {
  useSelect({ url: CLINIC_LIST, data: doctorId }, { value: 'id', label: 'clinic_name' }).then((data) => {
    cliniclist.value = data
    if (data.options.length > 0) {
      clinic_id.value = data.options[0].value
    }
    getClinicSession(clinic_id.value);
    getRequest({ url: LISTING_URL, id: { clinic_id: clinic_id.value, doctor_id: doctorId } }).then((res) => {
      if (res.status) {
        if (res.data != '') {
  ...
  getRequest({ url: CLINIC_SESSION_LIST, id: clinic_id.value }).then((res) => {
    if (res.status) {
      if (res.data != '') {
        ...
        console.log(clinicHolidays.value);
      } else {
        weekdays.value = defaultData();
      }
    }
...
const ClinicSelect = (e) => {
  ...
  const doctorId = currentId.value
