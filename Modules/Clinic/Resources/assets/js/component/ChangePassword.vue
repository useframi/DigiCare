<template>
    <form @submit.prevent="formSubmit">
      <div class="offcanvas offcanvas-end offcanvas-booking" id="doctor_change_password" aria-labelledby="form-offcanvasLabel">
  ...
  import { CHANGE_PASSWORD_URL } from '../constant/doctor'
  ...
  const currentId = useModuleId(() => {}, 'doctor_assign')
  ...
  const formSubmit = handleSubmit((values) => {
    values.doctor_id = currentId.value
  ...
  const reset_datatable_close_offcanvas = (res) => {
    if (res.status) {
      ...
      bootstrap.Offcanvas.getInstance('#doctor_change_password').hide()
  ...
  useOnOffcanvasHide('doctor_change_password', () => setFormData(defaultData()))
  </script>
